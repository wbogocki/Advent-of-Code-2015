...
function Reverse(array)
    local out = {}
    for i = #array, 1, -1 do table.insert(out, array[i]) end
    return out
end
...
function CheckConfiguration(packages, groups)
    -- Check if groups contain any duplicate packages

    local set = {}
    for i = 1, #groups do
        for _, element in ipairs(groups[i]) do set[element] = 1 end
    end
...
function MatchRemainingGroups(packages, possible_groups, group)
    ...
    for i = 1, remaining do
        for j = 1, i do
            for k = 1, j do
                if i + j + k == remaining then
                    table.insert(group_sizes, {g2 = i, g3 = j, g4 = k})
                end
            end
        end
    end
    ...
    --[[
    This will result in a MUCH better case for iterating the arrays below. We want to check group sizes like:

        #group, 10, 1, 1

    first, instead of group sizes like:

        #group, 4, 4, 4

    because in the second case we'll end up with a lot of repeating packages, which will explode the number of iterations we'll likely have to do.
    ]] --
    group_sizes = Reverse(group_sizes)
    ...
    for _, sizes in ipairs(group_sizes) do
        for _, g2 in ipairs(possible_groups[sizes.g2]) do
            for _, g3 in ipairs(possible_groups[sizes.g3]) do
                for _, g4 in ipairs(possible_groups[sizes.g4]) do
                    if CheckConfiguration(packages, {group, g2, g3, g4}) then
                        return {g2 = g2, g3 = g3, g4 = g4}
                    end
                end
            end
        end
            ...
            if configuration then
                return {
                    ...
                    groups = {
                        ...
                        Packages(packages, configuration.g4)
                    }
                }
            end
        end
...
local group_weight = Sum(packages) / 4
